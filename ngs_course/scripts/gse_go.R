...
cnetplot(egs_genename, categorySize = "NES", foldChange = dat_sort, font.size = 23, showCategory = 20)
dev.off()

# Enrichment map
a <- pairwise_termsim(egs, method = "JC", semData = NULL, showCategory = 200)
tiff(file = paste("GO_", prefix, "_enrich_map.tiff", sep =""), units = 'in', width = 10, height = 10, res = 200)
emapplot(a, showCategory = 23, color = "NES", font.size = 20)
dev.off()
...
jpeg(file = paste(prefix, "_heatmap.jpeg", sep =""), units = 'in', width = 20, height = 10, res = 300)
